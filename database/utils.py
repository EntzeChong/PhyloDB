...
def catAlphaDF(qs1, taxaDict, metaDict):
...
def quantAlphaDF(qs1, taxaDict, metaDict):
    finalDF = pd.DataFrame()
    metaDF = pd.DataFrame()
    final_fieldList = []
    for key, value in metaDict.items():
    ...
    for key, value in taxaDict.items():
        if type(value) is unicode:
            if key == 'Kingdom':
                ...
                field = 'speciesid'
                qs1 = ProfileSpecies.objects.filter(sampleid__in=myset).filter(Q(**{field: value})).values('sampleid', 'speciesid', 'speciesid__speciesName', 'count', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'speciesid', 'speciesid__speciesName', 'count', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                taxaDF['taxa'] = 'Species'
                taxaDF['rank'] = 7
                taxaDF.rename(columns={'speciesid': 'taxa_id', 'speciesid__speciesName': 'taxa_name'}, inplace=True)
                mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
            ...
            if key == 'Kingdom':
                for item in value:
                    field = 'kingdomid'
                    qs1 = ProfileKingdom.objects.filter(sampleid__in=myset).filter(Q(**{field: item})).values('sampleid', 'kingdomid', 'kingdomid__kingdomName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                    taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'kingdomid', 'kingdomid__kingdomName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                    taxaDF['taxa'] = 'Kingdom'
                    taxaDF['rank'] = 1
                    taxaDF.rename(columns={'kingdomid': 'taxa_id', 'kingdomid__kingdomName': 'taxa_name'}, inplace=True)
                    mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
                ...
                for item in value:
                    field = 'phylaid'
                    qs1 = ProfilePhyla.objects.filter(sampleid__in=myset).filter(Q(**{field: item})).values('sampleid', 'phylaid', 'phylaid__phylaName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                    taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'phylaid', 'phylaid__phylaName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                    taxaDF['taxa'] = 'Phyla'
                    taxaDF['rank'] = 2
                    taxaDF.rename(columns={'phylaid': 'taxa_id', 'phylaid__phylaName': 'taxa_name'}, inplace=True)
                    mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
                ...
                for item in value:
                    field = 'classid'
                    qs1 = ProfileClass.objects.filter(sampleid__in=myset).filter(Q(**{field: item})).values('sampleid', 'classid', 'classid__className', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                    taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'classid', 'classid__className', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                    taxaDF['taxa'] = 'Class'
                    taxaDF['rank'] = 3
                    taxaDF.rename(columns={'classid': 'taxa_id', 'classid__className': 'taxa_name'}, inplace=True)
                    mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
                ...
                for item in value:
                    field = 'orderid'
                    qs1 = ProfileOrder.objects.filter(sampleid__in=myset).filter(Q(**{field: item})).values('sampleid', 'orderid', 'orderid__orderName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                    taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'orderid', 'orderid__orderName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                    taxaDF['taxa'] = 'Order'
                    taxaDF['rank'] = 4
                    taxaDF.rename(columns={'orderid': 'taxa_id', 'orderid__orderName': 'taxa_name'}, inplace=True)
                    mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
                ...
                for item in value:
                    field = 'familyid'
                    qs1 = ProfileFamily.objects.filter(sampleid__in=myset).filter(Q(**{field: item})).values('sampleid', 'familyid', 'familyid__familyName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                    taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'familyid', 'familyid__familyName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                    taxaDF['taxa'] = 'Family'
                    taxaDF['rank'] = 5
                    taxaDF.rename(columns={'familyid': 'taxa_id', 'familyid__familyName': 'taxa_name'}, inplace=True)
                    mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
                ...
                for item in value:
                    field = 'genusid'
                    qs1 = ProfileGenus.objects.filter(sampleid__in=myset).filter(Q(**{field: item})).values('sampleid', 'genusid', 'genusid__genusName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                    taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'genusid', 'genusid__genusName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                    taxaDF['taxa'] = 'Genus'
                    taxaDF['rank'] = 6
                    taxaDF.rename(columns={'genusid': 'taxa_id', 'genusid__genusName': 'taxa_name'}, inplace=True)
                    mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
                ...
                for item in value:
                    field = 'speciesid'
                    qs1 = ProfileSpecies.objects.filter(sampleid__in=myset).filter(Q(**{field: item})).values('sampleid', 'speciesid', 'speciesid__speciesName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity')
                    taxaDF = pd.DataFrame.from_records(qs1, columns=['sampleid', 'speciesid', 'speciesid__speciesName', 'rel_abund', 'count', 'rel_abund', 'rich', 'diversity'])
                    taxaDF['taxa'] = 'Species'
                    taxaDF['rank'] = 7
                    taxaDF.rename(columns={'speciesid': 'taxa_id', 'speciesid__speciesName': 'taxa_name'}, inplace=True)
                    mergeDF = pd.merge(metaDF, taxaDF, on='sampleid', how='outer')
    ...
    # Set datatypes
    finalDF[['x-value', 'count', 'rich', 'rel_abund', 'diversity']] = finalDF[['x-value', 'count', 'rich', 'rel_abund', 'diversity']].astype(float)
    return finalDF
...
def catBetaTaxaDF(metaDF, myset, taxaLevel):
    taxaDF = pd.DataFrame()
    if taxaLevel == 2:
